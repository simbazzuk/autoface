...
const reportReasons = new Set(["fake_identity", "harassment", "financial_request", "inappropriate_content", "spam", "other"]);
...
export async function POST(request: Request) {
  try {
    ...
    if (!matchId || !action || !["unmatch", "block", "report"].includes(action)) throw new Error("INVALID_ACTION");
    ...
    if (action === "unmatch") {
      await matchRef.set({ status: "unmatched", endedBy: user.uid, endedAt: now, updatedAt: now }, { merge: true });
      await db.collection("conversations").doc(matchId).set({ status: "closed", closedReason: "unmatched", updatedAt: now }, { merge: true });
    ...
    if (action === "block") {
      ...
      await matchRef.set({ status: "blocked", endedBy: user.uid, endedAt: now, updatedAt: now }, { merge: true });
      await db.collection("conversations").doc(matchId).set({ status: "closed", closedReason: "blocked", updatedAt: now }, { merge: true });
      return NextResponse.json({ ok: true, action });
    }
    ...
    return NextResponse.json({ ok: true, action, reportId: reportRef.id });
  } catch (error) {
